...
describe("Utils.typifyPackageName", () => {
  ...
  it("should handle packages with namespace", () => {
    ...
    expect(typifyPackageName("@babel/code-frame")).toEqual(
      "@types/babel-code-frame"
    );
    expect(typifyPackageName("@babel/webpack-plugin")).toEqual(
      "@types/babel-webpack-plugin"
    );
  });
});
...
describe("Utils.installArguments", () => {
  ...
  test.each([
    ...
    [
      ...
      false,
      true,
      "install --save-dev react react-dom",
    ],
    ...
    (
      ...
      expect(installArguments(manager, packages, save, dev).join(" ")).toEqual(
        expected
      );
    }
  );
